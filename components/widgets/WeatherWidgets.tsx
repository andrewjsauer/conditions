...
export default function WeatherWidgets({
  ...
  const currentHourlyDetails = hourly.pop()
  ...
  return (
    <>
      ...
      <Card className="order-3 flex h-48 flex-col justify-between lg:order-2">
        ...
        <CardContent>
          <p>{formatSunTimeWithAMPM(current.sunset, cityTimezone)}</p>
        </CardContent>
        <CardFooter>
          <p>Sunrise: {formatSunTimeWithAMPM(current.sunrise, cityTimezone)}</p>
        </CardFooter>
      </Card>
      <Card className="order-4 h-48 xl:order-3">
        ...
        <CardContent className="flex justify-center p-0">
          <Compass
            speed={currentHourlyDetails.wind_speed}
            deg={currentHourlyDetails.wind_deg}
          />
        </CardContent>
      </Card>
      ...
      <Card className="order-6 flex h-48 flex-col justify-between">
        ...
        <CardContent>
          <p>
            {currentHourlyDetails.pop || 0}mm <br></br>in the last 3h
          </p>
        </CardContent>
        <CardFooter>
          <p>
            {currentHourlyDetails.pop !== 0
              ? currentHourlyDetails.pop <= 0.2
                ? "Light rain or drizzle. An umbrella may come in handy."
                : currentHourlyDetails.pop <= 2.5
                  ? "Moderate rain."
                  : "Heavy rain."
              : "Conditions are dry."}
          </p>
      ...
      <Card className="order-7 flex h-48 flex-col justify-between">
        ...
        <CardContent>
          <p>{Math.floor(currentHourlyDetails.feels_like)}&deg;</p>
        </CardContent>
        <CardFooter>
          <p>
            {currentHourlyDetails.feels_like < currentHourlyDetails.temp
      ...
      <Card className="order-8 flex h-48 flex-col justify-between">
        ...
        <CardContent>
          <p>{currentHourlyDetails.humidity}&deg;</p>
        </CardContent>
        <CardFooter>
          <p>
            {currentHourlyDetails.humidity < 40
              ? "Low humidity. It might feel dry."
              : currentHourlyDetails.humidity < 70
      ...
      <Card className="order-9 flex h-48 flex-col justify-between">
        ...
        <CardContent>
          <p>{currentHourlyDetails.visibility / 1000} km</p>
        </CardContent>
        <CardFooter>
          <p>
            {currentHourlyDetails.visibility >= 10
              ? "It's perfectly clear right now."
              : currentHourlyDetails.visibility >= 5
                ? "Good visibility."
      ...
      <Card className="order-10 flex h-48 flex-col justify-between">
        ...
        <CardContent>
          <p>{currentHourlyDetails.pressure} hPa</p>
        </CardContent>
        <CardFooter>
          <p>
            {currentHourlyDetails.pressure < 1000
              ? "Low pressure. Expect changes in the weather."
              : currentHourlyDetails.pressure >= 1000 &&
                  currentHourlyDetails.pressure <= 1010
